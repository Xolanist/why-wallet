...
/// Tx Proof - the why713 based proof that can be made for any address that is a public key.
/// we would like to generalize why713 proof implementation to be used in why-wallet proof framework with changing
/// of the message to generate signature in receiver wallet.
/// in why713 proof signature is generated using json string of  slate; and after upgrade
...
impl TxProof {
	...
	pub fn verify_extract(
		...
		//for why713 display purpose. the destination needs to be onion address
...
///support why713 payment proof message
pub fn proof_ok(
	...
	if global::is_mainnet() {
		...
		for output in outputs {
			println!(
				"   {}: https://explorer.floonet.why.mw/#o{}",
				output.bright_magenta(),
				output
			);
		}
		println!("kernel:");
		println!(
			"   {}: https://explorer.floonet.why.mw/#k{}",
			kernel.bright_magenta(),
			kernel
		);
	}
	...
	println!("please use a why block explorer to verify this is the case.");
}

//to support why713 payment proof verification
fn verify_tx_proof(
	tx_proof: &TxProof,
) -> Result<
	(
