	...
	K: Keychain + 'ck,
{
	...
	/// Load transaction by UUID. Needed for why713
...
pub trait NodeClient: Send + Sync + Clone {
	...
	/// Note, single block required singe request. Don't abuse it much because why713 wallets using the same node
...
pub struct Context {
	...
	/// Output commitments, why713 payment proof support.
	#[serde(default)]
	pub output_commits: Vec<Commitment>,
	/// Input commitments, why713 payment proof support.
	#[serde(default)]
...
/// NOTE2!!! Serialize, Deserialize needed for why713, even no version is present. Json should ease the problem
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct CbData {
	/// Output
	pub output: Output,
