...
impl Swap {
	...
	pub fn get_time_start_lock(&self) -> i64 {
		...
		self.get_time_message_offers()
			+ std::cmp::max(
				self.get_timeinterval_why_lock(),
	...
	/// MWC redeem time
	pub fn get_time_why_redeem(&self) -> i64 {
		self.get_time_message_redeem() + self.redeem_time_sec as i64
	}

	/// MWC locking time
	pub fn get_time_why_lock(&self) -> i64 {
		// Add 10% for network instability
		self.get_time_why_redeem() + self.get_timeinterval_why_lock()
	}

	/// why refund time
	pub fn get_time_why_refund(&self) -> i64 {
		// Add 10% for network instability
		self.get_time_why_lock() + self.redeem_time_sec as i64
	}

	/// BTC/ETH lock time for script
	pub fn get_time_secondary_lock_script(&self) -> i64 {
		self.get_time_why_refund()
	...
	pub fn get_timeinterval_why_lock(&self) -> i64 {
		// adding extra 10% for chain instability
		self.why_confirmations as i64 * 60 * 11 / 10
	}
