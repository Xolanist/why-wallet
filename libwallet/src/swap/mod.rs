...
mod tests {
	...
	fn test_btc_refund_tx_lock() {
		...
		let mut swap = api_sell
			.create_swap_offer(
				...
				true, // why should be publisher first
				30,
				3,
				3600,
				3600,
	...
	fn test_swap_fsm() {
		...
		let lock_second_message_round_timelimit = START_TIME
			...
			+ MSG_EXCHANGE_TIME;
		let why_lock_time_limit = lock_second_message_round_timelimit
			+ MWC_CONFIRMATION as i64 * 60 * 11 / 10
			+ REDEEM_TIME;
		let btc_lock_time_limit = why_lock_time_limit
			+ REDEEM_TIME
			+ REDEEM_TIME
			+ MWC_CONFIRMATION as i64 * 60 * 11 / 10
			+ BTC_CONFIRMATION as i64 * 10 * 60 * 11 / 10;
		...
		assert_eq!(seller.swap.get_time_why_lock(), why_lock_time_limit);
		...
		let nc_nolock_state = nc.get_state();
		{
			...
			// Resetting why chain as lock tx was never published and retry
			nc.set_state(&nc_nolock_state);
...

		{
			...
			let tlim = buyer.swap.get_time_why_redeem();
			test_responds(
				&mut buyer,
				StateId::BuyerWaitingForRespondRedeemMessage,
...

		{
			...
			// Seller should try to get both BTC and MWC refund
			{
				...
				// Let's why chain to loos all data, it shoudn't affect anything at that stage
				nc.set_state(&nc_state_ready);
				assert_eq!(
					nc.get_kernel(
	...
	fn test_btc_chain_ops() {
		...
		let found = crate::swap::fsm::seller_swap::check_why_redeem(&mut swap_sell, &nc).unwrap();
		assert!(found);
