			...
			if kernel.0.to_vec().iter().any(|v| *v != 0) {
					...
					.get_kernel(kernel, Some(slate.height), None)?
				{
					Some((_tx_kernel, height, _mmr_index)) => {
						Some(why_tip.saturating_sub(height) + 1)
					}
					None => None,
				}
			} else {
				if outputs_ok {
					...
					match height {
						Some(h) => Some(why_tip.saturating_sub(h) + 1),
						None => None,
					}
				} else {
					None
		...
		match self.eth_swap_details(swap, eth_data.address_from_secret.clone()) {
			Ok((refund_time, erc20_token_addr, _, participant, value)) => {
				...
				&& swap.redeem_public.is_some()
				{
					...
					// convert why public key to ethereum public key format
	...
	E: EthNodeClient + 'a,
{
	...
	fn request_tx_confirmations(
		...
		let why_tip = self.node_client.get_chain_tip()?.0;

		let is_seller = swap.is_seller();

		let why_lock_conf =
			self.get_slate_confirmation_number(&why_tip, &swap.lock_slate, !is_seller)?;
		let why_redeem_conf =
			self.get_slate_confirmation_number(&why_tip, &swap.redeem_slate, is_seller)?;
		let why_refund_conf =
			self.get_slate_confirmation_number(&why_tip, &swap.refund_slate, !is_seller)?;
