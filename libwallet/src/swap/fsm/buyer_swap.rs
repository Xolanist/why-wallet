		...
		match input {
			...
			Input::Check => {
				...
				if !swap.seller_lock_first {
					...
					let conf = tx_conf.why_lock_conf.unwrap_or(0);
	...
	C: NodeClient + 'a,
{
	...
	fn get_eta(&self, swap: &Swap) -> Option<StateEtaInfo> {
		Some(StateEtaInfo::new("Redeem MWC").end_time(swap.get_time_why_redeem()))
	}
	fn is_cancellable(&self) -> bool {
		true
	}
	...
	fn process(
		...
		let time_limit = swap.get_time_why_redeem();
		match input {
			Input::Cancel => {
				swap.add_journal_message(JOURNAL_CANCELLED_BY_USER.to_string());
				Ok(StateProcessRespond::new(StateId::BuyerWaitingForRefundTime))
			...
			Input::Check => {
				...
				if tx_conf.why_redeem_conf.unwrap_or(0) > 0 {
					return Ok(StateProcessRespond::new(
	...
	fn process(
		...
		match input {
			Input::Check => {
				...
				let conf = tx_conf.why_redeem_conf.unwrap_or(0);
				if conf >= swap.why_confirmations {
					// We are done
					swap.add_journal_message(
				...
				if tx_conf.why_redeem_conf.is_none()
	...
	fn process(
		...
		match input {
			Input::Check => {
				...
				debug_assert!(tx_conf.why_redeem_conf.is_none());
