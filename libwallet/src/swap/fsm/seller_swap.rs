	...
	C: NodeClient + 'a,
{
	...
	fn process(
		...
		match input {
			...
			Input::Check => {
				// Check if why lock is already done
				if tx_conf.why_lock_conf.is_some() {
...
impl<'a, K: Keychain> State for SellerWaitingForLockConfirmations<'a, K> {
	...
	fn process(
		...
		match input {
			...
			Input::Check => {
				let why_lock = tx_conf.why_lock_conf.unwrap_or(0);
				...
				if why_lock < swap.why_confirmations || !secondary_confirmed {
					...
					.action(Action::WaitForLockConfirmations {
						why_required: swap.why_confirmations,
						why_actual: why_lock,
						currency: swap.secondary_currency,
				...
				if why_lock < swap.why_confirmations {
					if why_lock == 0
						&& swap.posted_lock.clone().unwrap_or(0)
							< swap::get_cur_time() - super::state::POST_MWC_RETRY_PERIOD
					{
					...
					.action(Action::WaitForMwcConfirmations {
						...
						required: swap.why_confirmations,
						actual: why_lock,
					})
					.time_limit(time_limit));
				}
		...
		match input {
			Input::Check => {
				...
				if !swap.redeem_slate.tx.kernels().is_empty() {
					if check_why_redeem(swap, &*self.node_client)? {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
// Return true if why redeemed by Buyer. So we are good to claim BTC
pub(crate) fn check_why_redeem<C: NodeClient>(
	swap: &mut Swap,
	node_client: &C,
...
fn calc_why_unlock_time(swap: &Swap, tip: &u64) -> i64 {
		...
		if let Ok((height, _, _)) = self.node_client.get_chain_tip() {
			Some(
				StateEtaInfo::new("Wait For Buyer to redeem MWC")
					.end_time(calc_why_unlock_time(swap, &height)),
			)
		} else {
			...
			Input::Check => {
				...
				Ok(
					StateProcessRespond::new(StateId::SellerWaitingForBuyerToRedeemMwc)
						...
						.time_limit(calc_why_unlock_time(swap, &height)),
				)
			}
			_ => Err(ErrorKind::InvalidSwapStateInput(format!(
...
fn post_refund_if_possible<C: NodeClient>(
	...
	if height > swap.refund_slate.lock_height
		&& tx_conf.why_redeem_conf.is_none()
		&& tx_conf.why_refund_conf.is_none()
	{
	...
	C: NodeClient + 'a,
{
	...
	fn get_eta(&self, swap: &Swap) -> Option<StateEtaInfo> {
		if let Ok((height, _, _)) = self.node_client.get_chain_tip() {
			let start_time_limit = calc_why_unlock_time(swap, &height);
			Some(
	...
	fn process(
		...
		match input {
			Input::Check => {
				// Check if why lock is already done
				if tx_conf.why_refund_conf.is_some() {
					// already published.
					return Ok(StateProcessRespond::new(
				...
				if tx_conf.why_redeem_conf.is_some() {
			...
			Input::Execute => {
				// Executing the MWC lock transaction
				// Posting the transaction
				debug_assert!(tx_conf.why_refund_conf.is_none());
...
impl State for SellerWaitingForRefundConfirmations {
	...
	fn process(
		...
		match input {
			Input::Check => {
				// Check if why lock is already done
				if tx_conf.why_refund_conf.is_none() {
					if tx_conf.why_redeem_conf.is_some() {
				...
				let refund_conf = tx_conf.why_refund_conf.unwrap_or(0);
				if refund_conf > swap.why_confirmations {
					// already published.
