...
impl Network {
	...
	/// Constructor from why-node ChainTypes
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		let disp = match &self {
			...
			Action::WaitForLockConfirmations {
				...
				let why_str = if why_actual >= why_required {
					"MWC are locked".to_string()
				}
				else {
					format!("MWC {}/{}", why_actual, why_required)
				};
				...
				format!("Locking, waiting for confirmations. {}; {}", why_str, sec_str)
			}
			Action::SellerWaitForBuyerRedeemPublish {
				why_tip,
				lock_height,
			} => format!("Waiting for Buyer to redeem MWC. If we do not receive a response, a refund will be posted in {} minutes",(lock_height.saturating_sub(*why_tip))),
			Action::WaitForMwcRefundUnlock {
				why_tip,
				lock_height,
			} => {
				let blocks_left = lock_height.saturating_sub(*why_tip);
