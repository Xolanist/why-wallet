	...
	K: Keychain + 'a,
{
	...
	let context = tx::add_output_to_slate(
		...
		None,
		&parent_key_id,
		0, // Participant 0 for why713 compatibility
		message,
		true,
		use_test_rng,
		num_outputs,
	...
	{
		let mut batch = w.batch(keychain_mask)?;
		// Participant id is 0 for why713 compatibility
		batch.save_private_context(slate.id.as_bytes(), 0, &context)?;
		batch.commit()?;
	}
	...
	K: Keychain + 'a,
{
	...
	let mut context = tx::add_inputs_to_slate(
		...
		args.selection_strategy_is_use_all,
		&parent_key_id,
		1, // Participant id 1 for why713 compatibility
		message,
		false,
		use_test_rng,
		&None,
	...
	{
		let mut batch = w.batch(keychain_mask)?;
		// Participant id 1 for why713 compatibility
...
/// Finalize slate
/// Context needed for why713 proof of sending funds through whymqs
		...
		Err(_) => {
			if let Some(ref s) = status_send_channel {
				let _ = s.send(StatusMessage::Warning(
					"Unable to contact why-node".to_owned(),
				));
			}
