	...
	K: Keychain + 'a,
{
	...
	let (outputs, total, amount, fee) = if !(params.dry_run && params.why_amount == 0) {
	...
	for sw_id in &swap_id {
		...
		if do_check && !swap.state.is_final_state() {
			...
			result.push(SwapListInfo {
				...
				why_amount: core::amount_to_hr_string(swap.primary_amount, true),
	...
	K: Keychain + 'a,
{
	...
	let why_tip = node_client.get_chain_tip()?.0;
	if why_tip == 0 {
		return Err(ErrorKind::Generic(
	...
	if swap.is_seller() {
		...
		} else if tx_conf.why_refund_conf.is_some() {
			swap.state = StateId::SellerWaitingForRefundConfirmations;
		} else if tx_conf.why_lock_conf.is_some() {
