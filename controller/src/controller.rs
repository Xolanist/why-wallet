	...
	K: Keychain + 'static,
{
	...
	// why-wallet doesn have this field allways None. we don't want why-wallet to be able to process them.
	// Autoinvoice
	...
	fn process_incoming_slate(
		...
		if slate.num_participants > slate.participant_data.len() {
			...
			if slate.tx.inputs().len() == 0 {
				// why-wallet doesn't support invoices
...
pub fn init_start_whymqs_listener<L, C, K>(
	wallet: Arc<Mutex<Box<dyn WalletInst<'static, L, C, K>>>>,
	...
	K: Keychain + 'static,
{
	...
	//start whymqs listener
	start_whymqs_listener(wallet, mqs_config, wait_for_thread, keychain_mask, true)
		.map_err(|e| ErrorKind::GenericError(format!("cannot start mqs listener, {}", e)).into())
}

/// Start the mqs listener
pub fn start_whymqs_listener<L, C, K>(
	wallet: Arc<Mutex<Box<dyn WalletInst<'static, L, C, K>>>>,
	mqs_config: MQSConfig,
	wait_for_thread: bool,
	keychain_mask: Arc<Mutex<Option<SecretKey>>>,
	...
	K: Keychain + 'static,
{
	if grin_wallet_impls::adapters::get_whymqs_brocker().is_some() {
		return Err(
			ErrorKind::GenericError("whymqs listener is already running".to_string()).into(),
		);
	}
	...
	info!(
		"starting whymqs listener for {}:{}...",
		mqs_config.whymqs_domain, mqs_config.whymqs_port
	);
	info!(
		"the addres index is {}... ",
	...
	let whymqs_domain = mqs_config.whymqs_domain;
	let whymqs_port = mqs_config.whymqs_port;

	let whymqs_secret_key =
		controller_derive_address_key(wallet.clone(), keychain_mask.lock().as_ref())?;
	let why_pub_key = crypto::public_key_from_secret_key(&whymqs_secret_key)?;

	let whymqs_address = MWCMQSAddress::new(
		proofaddress::ProvableAddress::from_pub_key(&why_pub_key),
		Some(whymqs_domain.clone()),
		Some(whymqs_port),
	);

	let controller = Controller::new(
		&whymqs_address.get_stripped(),
		wallet.clone(),
		keychain_mask,
		None,
		print_to_log,
	...
	let whymqs_publisher = MWCMQPublisher::new(
		whymqs_address.clone(),
		&whymqs_secret_key,
		whymqs_domain,
		whymqs_port,
		print_to_log,
	...
	controller.set_publisher(Box::new(whymqs_publisher.clone()));

	let whymqs_subscriber = MWCMQSubscriber::new(&whymqs_publisher);

	let mut cloned_subscriber = whymqs_subscriber.clone();

	let thread = thread::Builder::new()
		.name("whymqs-broker".to_string())
		.spawn(move || {
			if let Err(e) = cloned_subscriber.start() {
				let err_str = format!("Unable to start whymqs controller, {}", e);
				error!("{}", err_str);
				panic!("{}", err_str);
			}
		})
		.map_err(|e| ErrorKind::GenericError(format!("Unable to start whymqs broker, {}", e)))?;

	// Publishing this running MQS service
	crate::impls::init_whymqs_access_data(whymqs_publisher.clone(), whymqs_subscriber.clone());

	if wait_for_thread {
		let _ = thread.join();
	}

	Ok((whymqs_publisher, whymqs_subscriber))
}
	...
	K: Keychain + 'static,
{
	...
	if api_secret.is_some() {
		let api_basic_auth =
			"Basic ".to_string() + &to_base64(&("why:".to_string() + &api_secret.unwrap()));
