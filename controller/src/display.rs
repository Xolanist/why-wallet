	...
	if !validated {
		println!(
			"\nWARNING: Wallet failed to verify data. \
			 ...
			 (is your `why server` offline or broken?)"
		);
	}
	Ok(())
}
...
pub fn txs(
	...
	if show_full_info {
		...
		// 'short' format is used by why 713 wallet
		table.set_titles(row![
			bMG->"Id",
			bMG->"Type",
	...
	for t in txs {
		...
		// why713 (short) representation of ID
...
pub fn swap_trade(
	...
	println!(
		...
		swap.why_confirmations.to_string().bold().yellow(),
	...
	if tx_conf.why_tip < swap.refund_slate.lock_height {
		let why_lock_sec = (swap.refund_slate.lock_height - tx_conf.why_tip) * 60;
		let sel_lock_h = why_lock_sec / 3600;
		let sel_lock_m = (why_lock_sec % 3600) / 60;
