...
fn accounts_test_impl(test_dir: &'static str) -> Result<(), wallet::Error> {
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |api, m| {
		...
		assert_eq!(wallet1_info.last_confirmed_height, 0); // why false case must be first. scan will update all accounts
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |api, m| {
		...
		let (_, wallet1_info) = api.retrieve_summary_info(m, false, 1)?;
		assert_eq!(wallet1_info.last_confirmed_height, 12); // for why no failure case. we are already updated for all accounts. Refresh comes from prev update
		let (wallet1_refreshed, wallet1_info) = api.retrieve_summary_info(m, true, 1)?;
		assert!(wallet1_refreshed);
		assert_eq!(wallet1_info.last_confirmed_height, 12);
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |api, m| {
		let (_, wallet1_info) = api.retrieve_summary_info(m, false, 1)?;
		assert_eq!(wallet1_info.last_confirmed_height, 13); // why already updated that
	...
	wallet::controller::owner_single_use(Some(wallet2.clone()), mask2, None, |api, m| {
		...
		assert_eq!(wallet2_info.last_confirmed_height, 13); // why already update all accounts
		let (wallet2_refreshed, wallet2_info) = api.retrieve_summary_info(m, true, 1)?;
		assert!(wallet2_refreshed);
		assert_eq!(wallet2_info.last_confirmed_height, 13);
