...
impl NodeClient for HTTPNodeClient {
	...
	fn get_libp2p_peers(&self) -> Result<Libp2pPeers, libwallet::Error> {
		debug!("Requesting libp2p peer connections from why-node");
		let params = json!([]);
		let res =
	...
	fn get_libp2p_messages(&self) -> Result<Libp2pMessages, libwallet::Error> {
		debug!("Requesting libp2p received messages from why-node");
		let params = json!([]);
...
mod tests {
	...
	fn run_node_stress_test() -> Result<(), libwallet::Error> {
		...
		let node_url = "https://why713.floonet.why.mw".to_owned();
