	...
	fn put_tx(
		...
		let out_slate = {
			if self.recipient.is_some() || self.slatepack_format {
				...
				warn!("Transaction contains features that require why-wallet 4.0.0 or later");
				warn!("Please ensure the other party is running why-wallet v4.0.0 or later before sending");
				...
				warn!("Transaction contains features that require why-wallet 3.0.0 or later");
				warn!("Please ensure the other party is running why-wallet v3.0.0 or later before sending");
				VersionedSlate::into_version_plain(slate.clone(), SlateVersion::V3).map_err(
					|e| ErrorKind::GenericError(format!("Failed convert Slate to Json, {}", e)),
				)?
			} else {
