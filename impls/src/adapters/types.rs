//The following is support mqs usage in why713
...
const DEFAULT_MWCMQS_DOMAIN: &str = "mqs.why.mw";
pub const DEFAULT_MWCMQS_PORT: u16 = 443;

const ADDRESS_REGEX: &str = r"^((?P<address_type>whymq|whymqs|https|http)://).+$";
...
const MWCMQ_ADDRESS_REGEX: &str = r"^(whymqs://)?(?P<public_key>[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{52})(@(?P<domain>[a-zA-Z0-9\.]+)(:(?P<port>[0-9]*))?)?$";
...
impl Display for MWCMQSAddress {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "whymqs://{}", self.address.public_key)?;
...
impl Address for MWCMQSAddress {
	...
	fn get_full_name(&self) -> String {
		"whymqs://".to_string() + &self.get_stripped()
	}
...
impl dyn Address {
	pub fn parse(address: &str) -> Result<Box<dyn Address>, Error> {
		...
		let address: Box<dyn Address> = match address_type.as_ref() {
			"whymqs" => Box::new(MWCMQSAddress::from_str(address)?),
