...
/// Init why mqs objects for the access.
pub fn init_whymqs_access_data(publisher: MWCMQPublisher, subscriber: MWCMQSubscriber) {
	MWCMQS_BROKER.write().replace((publisher, subscriber));
}

/// Init why mqs objects for the access.
pub fn get_whymqs_brocker() -> Option<(MWCMQPublisher, MWCMQSubscriber)> {
	MWCMQS_BROKER.read().clone()
}

/// Reset Broker (listener is stopped)
pub fn reset_whymqs_brocker() {
...
impl MwcMqsChannel {
	...
	fn send_tx_to_mqs(
		&self,
		slate: &Slate,
		whymqs_publisher: MWCMQPublisher,
...
impl SlateSender for MwcMqsChannel {
	...
	fn send_tx(
		...
		if let Some((whymqs_publisher, whymqs_subscriber)) = get_whymqs_brocker() {
			...
			whymqs_subscriber.set_notification_channels(&slate.id, tx_slate);
			let res = self.send_tx_to_mqs(slate, whymqs_publisher, rx_slate);
			whymqs_subscriber.reset_notification_channels(&slate.id);
			res
		} else {
...
impl SwapMessageSender for MwcMqsChannel {
	...
	fn send_swap_message(&self, message: &Message) -> Result<bool, Error> {
		if let Some((whymqs_publisher, _whymqs_subscriber)) = get_whymqs_brocker() {
			let (_ts_message, rs_message) = channel();

			self.send_swap_to_mqs(message, whymqs_publisher, rs_message)?;
...
impl MWCMQPublisher {
	...
	pub fn new(
		...
		Self {
			address,
			broker: MWCMQSBroker::new(whymqs_domain, whymqs_port, print_to_log, handler),
...
impl Publisher for MWCMQPublisher {
	fn post_slate(&self, slate: &Slate, to: &dyn Address) -> Result<(), Error> {
		let to_address_raw = format!("whymqs://{}", to.get_stripped());
			...
			.build()
		{
			...
			let response = client
				.post(&format!(
					...
					self.broker.whymqs_domain,
					self.broker.whymqs_port,
			...
			self.broker.stop();
			reset_whymqs_brocker();
			response_status
		} else {
			error!("Unable to stop whymqs threads");
			false
		}
	}
...
impl MWCMQSBroker {
	...
	fn post_slate(
		...
		if !self.is_running() {
			return Err(ErrorKind::ClosedListener("whymqs".to_string()).into());
		}
		let pkey = to.address.public_key()?;
		let skey = secret_key.clone();
		let version = slate.lowest_version();
		...
		if !response.is_ok() {
			return Err(ErrorKind::MqsInvalidRespose("whymqs connection error".to_string()).into());
		} else {
			...
			if !read_resp.is_ok() {
				return Err(ErrorKind::MqsInvalidRespose("whymqs i/o error".to_string()).into());
			} else {
				let data: Vec<&str> = resp_str.split(" ").collect();
				if data.len() <= 1 {
					return Err(ErrorKind::MqsInvalidRespose("whymqs".to_string()).into());
				} else {
					...
					if !last_seen.is_ok() {
						...
						if last_seen > 10000000000 {
							self.do_log_warn(format!("\nWARNING: [{}] has not been connected to whymqs recently. This user might not receive the slate.",
													 to.get_stripped()));
						} else if last_seen > 150000 {
							let seconds = last_seen / 1000;
							self.do_log_warn(format!("\nWARNING: [{}] has not been connected to whymqs for {} seconds. This user might not receive the slate.",
			...
			if !read_resp.is_ok() {
				...
				if data.len() <= 1 {
					...
					if !last_seen.is_ok() {
						...
						let last_seen = last_seen.unwrap();
						if last_seen > 10000000000 {
							println!("\nWARNING: [{}] has not been connected to whymqs recently. This user might not receive the swap message.",
									 to.get_stripped());
						} else if last_seen > 150000 {
							let seconds = last_seen / 1000;
							println!("\nWARNING: [{}] has not been connected to whymqs for {} seconds. This user might not receive the swap message.",
									 to.get_stripped(), seconds);
						}
					}
				}
	...
	fn subscribe(&mut self, source_address: &ProvableAddress, secret_key: &SecretKey) -> () {
		let address = MWCMQSAddress::new(
			...
			Some(self.whymqs_domain.clone()),
			Some(self.whymqs_port),
		);
		...
		if is_error {
			print!(
				"ERROR: Failed to start whymqs subscriber. Error connecting to {}:{}",
				self.whymqs_domain, self.whymqs_port
			);
		} else {
			let mut is_error = false;
			...
			loop {
				...
				if !resp_result.is_ok() {
					...
					if captures.is_none() {
						...
						if connected {
							is_in_warning = true;
							self.do_log_warn(format!("\nWARNING: whymqs listener [{}] lost connection. Will try to restore in the background. tid=[{}]",
						...
						delcount = 0;
						self.do_log_warn(format!(
							"\nwhymqs listener started for [{}] tid=[{}]",
						...
						if !connected {
							if is_in_warning {
								self.do_log_info(format!(
									"INFO: whymqs listener [{}] reestablished connection. tid=[{}]",
									cloned_cloned_address.get_stripped(),
									nanoid
								));
								is_in_warning = false;
					...
					if !read_resp.is_ok() {
						...
						self.do_log_info(format!("io error occured while trying to connect to {}. Will sleep for 5 second and will reconnect.",
												 &format!("https://{}:{}", self.whymqs_domain, self.whymqs_port)));
						...
						for i in 1..lines.len() {
							...
							if params.len() >= 2 {
								...
								if index.is_some() {
									...
									if cloned_running.load(Ordering::SeqCst) {
										self.do_log_error(format!(
											"\nERROR: new login detected. whymqs listener will stop!"
										));
									}
									break; // stop listener
								} else {
