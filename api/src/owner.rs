	...
	K: Keychain,
{
	...
	/// `why-wallet.toml` file and the `wallet_data` directory that contains the wallet
	...
	///
	/// 1) The current directory, from which `why-wallet` or the main process was run, if it
	/// contains a `why-wallet.toml` file.
	...
	/// Create a `why-wallet.toml` configuration file in the top-level directory for the
	...
	/// path-related values in the optional configuration structs will be ignored.
	///
	/// The wallet files must not already exist, and ~The `why-wallet.toml` file must exist
