...
fn command_line_test_impl(test_dir: &str) -> Result<(), grin_wallet_controller::Error> {
	...
	let yml = load_yaml!("../src/bin/why-wallet.yml");
	let app = App::from_yaml(yml);

	// wallet init
	let arg_vec = vec!["why-wallet", "-p", "password", "init", "-h"];
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "account", "-c", "mining"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

	let arg_vec = vec!["why-wallet", "-p", "password", "account", "-c", "account_1"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "account", "-c", "account_2"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;

	// let's see those accounts
	let arg_vec = vec!["why-wallet", "-p", "password", "account"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "-a", "mining", "info"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "-a", "account_1", "info"];
	...
	grin_wallet_controller::controller::owner_single_use(
		...
		|api, m| {
			...
			assert_eq!(wallet1_info.amount_currently_spendable, 300_000_000); // grin: 10_000_000_000,  why 0.3 to nano
			Ok(())
		},
	)?;
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "scan", "-d"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "-a", "mining", "txs"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
	...
	// txs and outputs (mostly spit out for a visual in test logs)
	let arg_vec = vec!["why-wallet", "-p", "password", "-a", "mining", "outputs"];
	execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

	let arg_vec = vec!["why-wallet", "-p", "password", "txs"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;

	let arg_vec = vec!["why-wallet", "-p", "password", "outputs"];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
	...
	let arg_vec = vec!["why-wallet", "-p", "password", "txs", "-t", &tx_id[..]];
	execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
