...
use why_wallet::cmd::wallet_args;
...
macro_rules! setup_proxy {
	($test_dir: expr, $chain: ident, $wallet1: ident, $client1: ident, $mask1: ident, $wallet2: ident, $client2: ident, $mask2: ident) => {
		...
		let target = std::path::PathBuf::from(format!("{}/wallet1/why-wallet.toml", $test_dir));
		println!("{:?}", target);
		if !target.exists() {
		...
		let target = std::path::PathBuf::from(format!("{}/wallet2/why-wallet.toml", $test_dir));
		if !target.exists() {
...
pub fn config_command_wallet(
	...
	config_file_name.push("why-wallet.toml");
	if config_file_name.exists() {
		return Err(grin_wallet_controller::ErrorKind::ArgumentError(
			"why-wallet.toml already exists in the target directory. Please remove it first"
				.to_owned(),
		))?;
	}
...
> {
	...
	// legacy hack to avoid the need for changes in existing why-wallet.toml files
