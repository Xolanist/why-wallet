...
const COLORED_PROMPT: &'static str = "\x1b[36mwhy-wallet>\x1b[0m ";
const PROMPT: &'static str = "why-wallet> ";
	...
	loop {
		match reader.readline(PROMPT) {
			Ok(command) => {
				...
				// Just add 'why-wallet' to each command behind the scenes
				// so we don't need to maintain a separate definition file
				let augmented_command = format!("why-wallet {}", command);
				let args =
