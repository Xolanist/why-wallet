...
fn real_main() -> i32 {
	let yml = load_yaml!("why-wallet.yml");
	...
	let mut config = match config::initial_setup_wallet(&chain_type, current_dir, None, create_path)
	{
		...
		Err(e) => match e {
			ConfigError::PathNotFoundError(m) => {
				println!("Wallet configuration not found at {}. (Run `why-wallet init` to create a new wallet)", m);
				return 0;
			}
			m => {
				println!("Unable to load wallet configuration: {} (Run `why-wallet init` to create a new wallet)", m);
				return 0;
			}
		},
	};
	...
	// Default derive index is 1 to match what why713 has by default...
	...
	let node_client = HTTPNodeClient::new(node_list, None)
		.expect("Unable create HTTP client for why-node connection");
