...
pub fn parse_swap_start_args(args: &ArgMatches) -> Result<SwapStartArgs, ParseError> {
	let why_amount = parse_required(args, "why_amount")?;
	let why_amount = core::core::amount_from_hr_string(why_amount);
	let why_amount = match why_amount {
		Ok(a) => a,
		Err(e) => {
			let msg = format!(
	...
	let why_lock = parse_required(args, "why_confirmations")?;
	let why_lock = parse_u64(why_lock, "why_confirmations")?;
	...
	K: keychain::Keychain + 'static,
{
	...
	match wallet_args.subcommand() {
		...
		("swap_create_from_offer", Some(args)) => {
			let why_amount = arg_parse!(parse_required(args, "file"));
			command::swap_create_from_offer(owner_api, km, why_amount.to_string())
		}
		("swap", Some(args)) => {
		...
		(cmd, _) => {
			return Err(ErrorKind::ArgumentError(format!(
				"Unknown wallet command '{}', use 'why help wallet' for details",
				cmd
			))
			.into());
		}
